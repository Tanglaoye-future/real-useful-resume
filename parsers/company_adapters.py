...
class XiaohongshuAdapter(BaseCompanyAdapter):
    ...
    def fetch_list(self, page: int) -> List[Dict[str, Any]]:
        ...
        try:
            resp = self.session.post(self.list_url, json=payload, timeout=self.timeout).json()
    ...
    def fetch_detail(self, position_id: str) -> Dict[str, Any]:
        pid = _norm(position_id)
        if not pid:
            return {}
        attempts = [
            (
                "https://job.xiaohongshu.com/websiterecruit/position/getPositionDetail",
                {"positionId": pid},
                "post",
            ),
            (
        ...
        for url, payload, method in attempts:
            try:
                if method == "post":
                    resp = self.session.post(url, json=payload, timeout=self.timeout).json()
                else:
                    resp = self.session.get(url, params=payload, timeout=self.timeout).json()
                data = resp.get("data") or {}
...
class BilibiliAdapter(BaseCompanyAdapter):
    company = "哔哩哔哩"
    ...
    detail_url_template = "https://jobs.bilibili.com/api/campus/position/detail/{position_id}"

    def __init__(self, page_size: int = 30, timeout: int = 20):
        self.page_size = page_size
        self.timeout = timeout
        self.session = requests.Session()
        self.headers = {
            "User-Agent": "Mozilla/5.0",
            "Referer": f"{self.base_url}/campus/positions",
            "Origin": self.base_url,
            "X-AppKey": "ops.ehr-api.auth",
            "X-UserType": "2",
            "Content-Type": "application/json",
        }
        self._csrf = _norm(os.getenv("BILIBILI_CSRF"))
        self.strict_mode = False

    def set_mode(self, strict_mode: bool):
    ...
    def _ensure_csrf(self) -> str:
        if self._csrf:
            return self._csrf
        try:
            self.session.get(f"{self.base_url}/campus/positions", headers={"User-Agent": "Mozilla/5.0"}, timeout=self.timeout)
            resp = self.session.get(
                f"{self.base_url}/api/auth/v1/csrf/token",
                headers={"User-Agent": "Mozilla/5.0", "X-AppKey": "ops.ehr-api.auth", "X-UserType": "2"},
                timeout=self.timeout,
            ).json()
            self._csrf = _norm((resp or {}).get("data"))
        except Exception:
            self._csrf = ""
        return self._csrf
    ...
    def fetch_list(self, page: int) -> List[Dict[str, Any]]:
        ...
        if csrf:
            headers["X-CSRF"] = csrf
        try:
            resp = self.session.post(self.list_url, json=payload, headers=headers, timeout=self.timeout).json()
            data = resp.get("data") or {}
            items = data.get("list") or data.get("records") or []
            return items if isinstance(items, list) else []
        except Exception:
    ...
    def fetch_detail(self, position_id: str) -> Dict[str, Any]:
        ...
        if csrf:
            headers["X-CSRF"] = csrf
        try:
            url = self.detail_url_template.format(position_id=position_id)
            resp = self.session.get(url, headers=headers, timeout=self.timeout).json()
            data = resp.get("data") or {}
    ...
    def parse(self, raw_item: Dict[str, Any]) -> Dict[str, str]:
        position_id = _norm(raw_item.get("id") or raw_item.get("positionId"))
        title = _norm(raw_item.get("positionName") or raw_item.get("name") or raw_item.get("title"))
        location_raw = _norm(raw_item.get("workLocation") or raw_item.get("workLocationName") or raw_item.get("city"))
        desc = _norm(raw_item.get("positionDescription") or raw_item.get("description"))
        req = _norm(raw_item.get("positionRequire") or raw_item.get("positionDemand") or raw_item.get("jobRequire"))
        jd_raw = _norm(f"{desc} 岗位要求：{req}")
        raw_tags = _norm(
            f"{raw_item.get('postCodeName') or ''} {raw_item.get('positionTypeName') or ''} {raw_item.get('campusProjectId') or ''}"
        )
        return {
            "external_job_id": position_id,
            "title": title,
            ...
            "jd_raw": jd_raw,
            "update_time": _norm(raw_item.get("pushTime") or raw_item.get("publishTime") or raw_item.get("updateTime")),
            "publish_time": _norm(raw_item.get("pushTime") or raw_item.get("publishTime")),
            "deadline": _norm(raw_item.get("webApplyEndTime") or raw_item.get("deadline")),
            "raw_tags": raw_tags,
            "url": f"{self.base_url}/campus/positions/{position_id}" if position_id else f"{self.base_url}/campus",
            "collect_time": time.strftime("%Y-%m-%d %H:%M:%S"),
            "source": self.source,
            "recruit_type": _norm(raw_item.get("positionTypeName") or raw_item.get("postCodeName")),
            "project_name": _norm(raw_item.get("campusProjectId")),
        }

    def get_27_signal(self, parsed_item: Dict[str, str]) -> Dict[str, Any]:
        text = _norm(f"{parsed_item.get('project_name', '')} {parsed_item.get('jd_raw', '')}")
        return {
            "has_explicit_year": any(k in text for k in ["2027", "27届", "2026.9", "2027.8"]),
            "year_text": text,
            "project_name": parsed_item.get("project_name", ""),
        }
