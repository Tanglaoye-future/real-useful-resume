...
- **高价值岗位筛选**: 基于公司分级、岗位含金量、投递成功率的多维度评估
...
│       ├── official/             # 官网数据
│       └── merged/               # 合并数据
├── run_data_pipeline.py          # ⭐ 数据管道运行脚本（新增）
├── test_pipeline.py              # ⭐ 数据管道测试脚本（新增）
...
# 处理特定数据源
python run_data_pipeline.py --mode source --source 51job
```
...
rules = FilterRule(
    locations=["上海", "Shanghai"],      # 目标地点
    min_education="本科",               # 最低学历
    min_salary=150,                     # 最低薪资(元/天)
    blacklist_companies=["外包", "劳务派遣"]  # 黑名单
)
```
...
config = ValueFilterConfig(
    min_company_tier=CompanyTier.TIER_3,  # 最低公司等级(中厂)
...
# 获取结果
high_value_jobs = result['high_value_jobs']
medium_value_jobs = result['medium_value_jobs']
```
...
| 等级 | 名称 | 示例公司 |
|------|------|----------|
| TIER_1 | 一线大厂 | 字节跳动、阿里巴巴、腾讯、美团、京东、拼多多、快手、小红书 |
| TIER_2 | 二线大厂 | 哔哩哔哩、携程、滴滴、蔚来、理想、小鹏、贝壳、知乎 |
| TIER_3 | 中厂 | BOSS直聘、猎聘、拉勾、斗鱼、虎牙、Keep、喜马拉雅 |
| TIER_4 | 小厂 | 其他公司 |
| UNKNOWN | 未知 | 无法识别的公司 |
