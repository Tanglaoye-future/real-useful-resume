...
def bootstrap_cookies():
    try:
        r = SESSION.get('https://www.shixiseng.com/', timeout=10)
        cookie_names = [c.name for c in SESSION.cookies]
        logger.info(f'bootstrap cookies count={len(cookie_names)} names={cookie_names}')
    except requests.RequestException as e:
        logger.warning(f'bootstrap cookies failed: {e}')
