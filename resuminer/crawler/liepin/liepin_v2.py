        ...
        with sync_playwright() as p:
            ...
            for page_num in range(1, self.max_pages + 1):
                try:
                    ...
                    # 解析职位
                    page_jobs = []
                    for idx, card in enumerate(job_cards[:20]):  # 每页最多20条
                        try:
                            job = self._parse_job_card(card, page)
                            if job:
                                page_jobs.append(job)
                                if idx < 3:  # 只打印前3条的调试信息
                                    logger.info(f"[Liepin V2] Parsed job {idx+1}: {job.get('job_name', 'N/A')[:30]}...")
                        except Exception as e:
                            logger.warning(f"[Liepin V2] Failed to parse card {idx}: {e}")
                            import traceback
                            logger.debug(f"[Liepin V2] Traceback: {traceback.format_exc()}")
                            continue
                    ...
                    if page_jobs:
                        ...
                        logger.warning(f"[Liepin V2] Page {page_num}: No jobs parsed from {len(job_cards)} cards")
    ...
    def _parse_job_card(self, card, page):
        ...
        try:
            ...
            if isinstance(card, dict):
                ...
                # 尝试多种方式获取标题
                title_selectors = [".job-title", ".title", "h3", "a", ".job-name"]
                ...
                # 提取公司名
                company_selectors = [".company-name", ".comp-name", ".company", ".job-company"]
                for sel in company_selectors:
                    company_elem = card.query_selector(sel)
                    if company_elem:
                ...
                # 提取薪资
                salary_selectors = [".salary", ".pay", ".money", ".job-salary"]
                for sel in salary_selectors:
                    salary_elem = card.query_selector(sel)
                    if salary_elem:
                ...
                # 提取地点
                location_selectors = [".job-area", ".area", ".location", ".job-location"]
                for sel in location_selectors:
                    location_elem = card.query_selector(sel)
                    if location_elem:
