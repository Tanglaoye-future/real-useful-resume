...
class PlaywrightContext:
    ...
    async def _init_detail_pages(self):
        ...
        for platform_key, seed_url in [
            ...
            ("yingjiesheng", "https://www.yingjiesheng.com/shanghai/"),
        ]:
            try:
                page = await self.context.new_page()
    ...
    async def fetch_detail_html(self, platform: str, url: str) -> dict:
        ...
        async with self._detail_lock:
            ...
            try:
                ...
                if not blocked and platform not in ("shixiseng", "yingjiesheng"):
                    # For shixiseng/yingjiesheng we trust the title check only:
                    # - shixiseng embeds 'traceid' and WAF tokens in every normal page
                    # - captcha pages on these sites reliably show a distinct title
                    snippet = html[:3000].lower()
                    blocked = bool(re.search(r"captcha|traceid|请完成验证|访问受限", snippet))
    ...
    try:
        if platform == "boss" and action == "get_stoken":
            ...
            async with pw_ctx._liepin_search_lock:
                ...
                # Extract pagination from the raw _liepin_search_resp captured earlier.
                # We stored `data` from the JS intercept; dig out pagination regardless
                # of nesting depth so the spider can honour hasNext / totalPage.
                _pagination: dict = {}
                if isinstance(data, dict):
                    _d1 = data.get("data") or {}
                    _pagination = (
                        _d1.get("pagination")
                        or _d1.get("pager")
                    ...
                    if not isinstance(_pagination, dict):
                        _pagination = {}
