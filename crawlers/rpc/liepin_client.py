...
class LiepinCrypto:
    ...
    def fetch_detail_html(self, url: str):
        """Fetch a job detail page HTML via the local RPC browser context.

        This avoids direct HTTP scraping and reuses the authenticated Playwright
        session (cookies) maintained by the RPC server.
        """
        try:
            if not url:
                return {"html": "", "blocked": False, "error": "url_required"}
            rpc = f"{self.rpc_url}/invoke/liepin/fetch_detail"
            payload = {"url": url}
            ...
            logger.error(f"[LiepinCrypto] fetch_detail_html exception: {e}")
